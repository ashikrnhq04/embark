 ...
 */
{
  document.addEventListener("DOMContentLoaded", function () {
    ...
    let notificationDisplay = document.querySelector("#notification-display");
    let closeNotificationDisplay =
      notificationDisplay.querySelector(".sidenav-close");
    let instances = M.Sidenav.init(notificationBar, {
      ...
      onCloseStart: function () {
        if (!closeNotificationDisplay) {
          return;
        }
        closeNotificationDisplay.click();
     ...
     */
    function addClass() {
      // add class to notification display
      if (!notificationDisplay.classList.contains("active")) {
        notificationDisplay.classList.add("active");
      } else {
        notificationDisplay.classList.remove("active");
      }
    }
    let viewnotification = M.Sidenav.init(notificationDisplay, {
      edge: "right",
      onOpenStart: addClass,
      onCloseStart: addClass,
    });
  });
}
/**
 ...
 */
{
  var collapseMenu = document.querySelectorAll(".collapsible.expandable");
 ...
 */
{
  let passBtns = document.querySelectorAll(".password .icons");
...
/**
 * hash link event disabled
 */
{
  let links = document.querySelectorAll("a[href='#']");
  function defaultEventDisable(link) {
    if (!link) {
      return;
    }
    link.preventDefault();
  }
  links.forEach((link) => link.addEventListener("click", defaultEventDisable));
}
